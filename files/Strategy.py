...
class Strategy():
    ...
    def isSellTrigger(self, price: float=0.0, price_exit: float=0.0, margin: float=0.0, change_pcnt_high: float=0.0, obv_pc: float=0.0, macdltsignal: bool=False) -> bool:
        ...
        if self.app.trailingStopLoss() != None and change_pcnt_high < self.app.trailingStopLoss() and (margin > 5 or margin < -10):
    ...
    def isWaitTrigger(self, margin: float=0.0):
        ...
        if self.state.action == 'SELL' and self.app.allowSellAtLoss() and (margin <= 5 or margin >= -10):
